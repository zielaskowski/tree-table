function lev(table) {
  // new $.fn.dataTable.ColReorder( table, { } ); //add colReorder extension
  // table.colReorder.move() //move button col to begining
  ...
  //styling
  table.tables().nodes().to$().removeClass("display").addClass("compact row-border");
  table.column(pathCol).visible(false);
  table.column(onoffCol).visible(false);
  table.column(buttonCol).header().innerHTML = "";

  //coloring, pointer and indentation
  let widgetId = table.nodes().to$().closest("div.datatables").attr("id");
  let d = JSON.parse(document.querySelector("script[data-for=" + widgetId + "]").innerText);
  let lev = table
  .column(pathCol)
  .data()
  .toArray()
